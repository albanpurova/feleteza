...
export default function RichTextEditor({
  ...
  const btn =
    "px-2.5 py-1 rounded text-sm text-brand-navy hover:bg-black/5 border border-transparent";
  ...
  return (
    <div className="rounded-lg border border-black/15 bg-white">
      ...
      <div className="flex flex-wrap items-center gap-1 border-b border-black/10 p-2">
        <button type="button" className={`${btn} font-bold`} onClick={() => exec("bold")} title="Trash">B</button>
        <button type="button" className={`${btn} italic`} onClick={() => exec("italic")} title="Pjerrët">I</button>
        <span className="mx-1 h-5 w-px bg-black/10" />
        <button type="button" className={btn} onClick={() => exec("formatBlock", "<h2>")}>Titull</button>
        <button type="button" className={btn} onClick={() => exec("formatBlock", "<h3>")}>Nëntitull</button>
        <button type="button" className={btn} onClick={() => exec("formatBlock", "<p>")}>Paragraf</button>
        <span className="mx-1 h-5 w-px bg-black/10" />
        <button type="button" className={btn} onClick={() => exec("insertUnorderedList")} title="Listë">• Listë</button>
        <button type="button" className={btn} onClick={() => exec("insertOrderedList")} title="Listë me numra">1. Listë</button>
        <span className="mx-1 h-5 w-px bg-black/10" />
        <button type="button" className={btn} onClick={addLink}>🔗 Lidhje</button>
